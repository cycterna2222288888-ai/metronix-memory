...
Everything else is a failure. STOP. Do NOT edit CLAUDE.md. Report exactly
...
update the MCP server registration written in prompt 1 to use it instead of
the shared key, restart Claude Code, then re-run this prompt. Do not attempt
to self-provision a credential or otherwise bypass this check.
