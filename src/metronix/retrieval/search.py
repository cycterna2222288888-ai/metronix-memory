...
from metronix.llm import chat_completion, chat_completion_with_retry  # TODO: async migration
from metronix.llm.telemetry import add_extra_metadata, update_retrieved_context
from metronix.observability.metrics import timed
...
from metronix.retrieval.query_classifier import classify_query, get_profile_weights
from metronix.retrieval.query_expansion import expand_query
from metronix.retrieval.routing import (
...
def detect_response_language(query: str) -> str:
    ...
    "Что такое Metronix?" (Russian question with an English proper noun)
...
def _extract_fast_signals(query: str) -> tuple[list[str], tuple[str, ...] | None]:
    ...
    from metronix.ingestion.processors.dates import get_dates_in_range
...
def _build_freshness_filter(settings: Settings | None):  # type: ignore[no-untyped-def]
    ...
    ``METRONIX_FRESHNESS_KB_SEARCH_FILTER_ENABLED=False`` (default) so the
...
async def fast_search(
    ...
    the ``metronix_search_fast`` MCP tool for routine lookups. It bypasses
    ...
    *,
    ...
    async def _persist_rag_trace() -> None:
        ...
        try:
            ...
            from metronix.storage.pg_connection import store_rag_trace_sync
    ...
    if _s.hyde_enabled:
        ...
        if is_vague:
            from metronix.retrieval.query_expansion import get_hyde_embedding
    ...
    if workspace_id:
        try:
            ...
            from metronix.storage.pg_connection import store_query_trace_sync
            ...
            if doc_stats:
                from metronix.storage.pg_connection import upsert_document_fetch_stats_sync
