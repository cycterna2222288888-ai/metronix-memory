...
def settings() -> Settings:
    return Settings(
        ...
        METRONIX_OPENAI_COMPAT_KEY="test-key-123",
    )
...
class TestModelsEndpoint:
    ...
    def test_list_models_returns_workspaces(self, mock_mgr, client):
        ...
        assert data["data"][0]["id"] == "metronix-rag-TEST_WS"
        assert data["data"][0]["owned_by"] == "metronix"
    ...
    def test_list_models_disabled_returns_404(self):
        settings = Settings(
            ...
            METRONIX_OPENAI_COMPAT_ENABLED=False,
        )
    ...
    def test_list_models_no_key_configured_returns_401(self):
        """When METRONIX_OPENAI_COMPAT_KEY is empty and no api_key_store, returns 401."""
        settings = Settings(
            METRONIX_ENV="development",
            METRONIX_OPENAI_COMPAT_KEY="",
            METRONIX_OPENAI_COMPAT_ENABLED=True,
        )
        app = create_app(settings)
        client = TestClient(app, raise_server_exceptions=False)
...
class TestChatCompletions:
    ...
    def test_non_streaming_returns_openai_format(self, mock_search, mock_mgr, client):
        ...
        assert data["model"] == "metronix-rag-TEST_WS"
    ...
    def test_unknown_model_returns_404(self, mock_mgr, client):
        ...
        r = client.post(
            "/v1/chat/completions",
            json={
                "model": "metronix-rag-NONEXISTENT",
    ...
    def test_empty_messages_returns_400(self, client):
        r = client.post(
            "/v1/chat/completions",
            json={"model": "metronix-rag-TEST_WS", "messages": []},
            headers={"Authorization": "Bearer test-key-123"},
        )
